...
avg_dens, = plt.plot(ts, np.sqrt((xs-xs2)**2 + (ys-ys2)**2 + (zs-zs2)**2)/1000.0 , 'g-', label = '500 x 500 km')
...
out2 = run_simulation(state = [r_earth + altitude_0, 0, 0, vx_0_1, vy_0_1, vz_0_1], second_state = [r_earth + altitude_0, 0, 0, vx_0_2, vy_0_2, vz_0_2], k_i_drag = -0.025, k_p_drag = -5000.0, threshold_drag = 100.0)
...
high_dens, = plt.plot(ts12, np.sqrt((xs12-xs22)**2 + (ys12-ys22)**2 + (zs12-zs22)**2)/1000.0 , 'b-', label = '600 x 600 km')

# Lower orbit

# Initial altitude
altitude_0 = 400 * 1000 # [m]
...
out3 = run_simulation(state = [r_earth + altitude_0, 0, 0, vx_0_1, vy_0_1, vz_0_1], second_state = [r_earth + altitude_0, 0, 0, vx_0_2, vy_0_2, vz_0_2], k_i_drag = -0.080, k_p_drag = -4500.0, threshold_drag = 500.0)
...
low_dens, = plt.plot(ts13, np.sqrt((xs13-xs23)**2 + (ys13-ys23)**2 + (zs13-zs23)**2)/1000.0 , 'r-', label = '400 x 400 km')
...
plt.title('Differential Drag Performance in Various Sun Synchronous Orbits')
plt.savefig('pi_controller_varied_orbit.png', bbox_inches='tight')
plt.show()
